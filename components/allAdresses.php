...
//we now select all adresses
$ids = array();
$id = $_SESSION["user"]->getId();
$sql = mysqli_query($succes, $sql = "SELECT * FROM `adress` WHERE 1");
$row = mysqli_num_rows($sql);
$compteur = 0;
while ($row > $compteur) {
    $result = mysqli_fetch_assoc($sql);
    $adresses[$compteur] = new adress($result["country"], $result["city"], $result["postal"], $result["adress"]);
    $ids[$compteur] = $result["userID"];
    $compteur = $compteur + 1;
}
// Close statement
mysqli_close($succes);
    ...
    <input type="text" id="indexofcolor" value='<?php echo $indexOfColor ?>'>
        ...
        <div class="myOrders">
            <table style="text-align: center; border: 1px solid black">
                <tbody>
                    <tr>
                        <td><strong>Owner's id</strong></td>
                    ...
                    <?php for ($i = 0; $i < $compteur; $i++) { ?>
                        <tr>
                            ...
                            <td><?php echo $adresses[$i]->getCountry() ?></td>
                            <td><?php echo $adresses[$i]->getCity() ?></td>
                            <td><?php echo $adresses[$i]->getPostal() ?></td>
                            <td><?php echo $adresses[$i]->getAdress() ?></td>
                        </tr>
                    <?php } ?>
                </tbody>
            </table>
