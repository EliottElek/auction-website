...
//we select all orders
$id = $_SESSION["user"]->getId();
$sql = mysqli_query($succes, $sql = "SELECT * FROM `orders` WHERE 1");
$row = mysqli_num_rows($sql);
$compteur = 0;
$orders = array();
while ($row > $compteur) {
    $result = mysqli_fetch_assoc($sql);
    $orders[$compteur] = new order($result["customerId"], $result["name"], $result["adress"], $result["orderdate"], $result["price"], $result["shippingdate"], $result["comment"], $result["id"]);
    $compteur = $compteur + 1;
}
// Close statement
mysqli_close($succes);
...
<head>
    ...
    <style>
        <?php include "../assets/myProfile.css" ?><?php include "../assets/myOrders.css" ?><?php include "../assets/AllOrders.css" ?><?php include "../assets/mainPage.css" ?>
    </style>
...
<body>
    ...
    <input type="text" id="indexofcolor" value='<?php echo $indexOfColor ?>'>
        ...
        <h3 id="welcome">ADMIN SESSION -  Orders</h3>
        <div class="myOrders">
            <table style="text-align: center; border: 1px solid black; margin-bottom:100px">
                <tbody>
                    <tr>
                    ...
                    <?php for ($i = 0; $i < $compteur; $i++) { ?>
                        <tr>
                            ...
                            <td><?php echo $orders[$i]->getAdress1() ?></td>
                            <td><?php echo $orders[$i]->getOrderDate() ?></td>
                            <td><?php echo $orders[$i]->getPrice() ?> £</td>
                            <!-- <?php if (getDiffDates($orders[$i]->getOrderDate()) <=-1 && getDiffDates($orders[$i]->getOrderDate()) > -2) { ?>
                                <td><strong style="color:orange">paid</strong></td>
                            <?php } else if (getDiffDates($orders[$i]->getOrderDate()) <= -2 && getDiffDates($orders[$i]->getOrderDate()) > -3) { ?>
                                <td><strong style="color:purple">in preparation</strong></td>
                                <?php } else if (getDiffDates($orders[$i]->getOrderDate()) <= -3 && getDiffDates($orders[$i]->getOrderDate()) > -4){ ?>
                                <td><strong style="color:blue">shipped</strong></td>
                            <?php } else if (getDiffDates($orders[$i]->getOrderDate()) <= -6) { ?>
  ...
  <footer style = "transform:translateY(1200%)">
...
<script>
    <?php include "../javascriptnavApp.js" ?>
</script>
<script>
    <?php include "../javascriptmainPage.js" ?>
</script>
<script>
  <?php include "../javascriptmyProfile.js" ?>
</script>
</html>