...
//loading all orders that user has made
$id = $_SESSION["user"]->getId();
$sql = mysqli_query($succes, $sql = "SELECT * FROM `orders` WHERE `customerId` LIKE $id");
...
<body>
  <section>
    ...
    <h3 id="welcome">Follow your orders</h3>
    <div class="myOrders">
        ...
        <tbody>
          ...
          <?php for ($i = 0; $i < $compteur; $i++) { ?>
            <tr>
              ...
              <!-- <?php if (getDiffDates($orders[$i]->getOrderDate()) <= -1 && getDiffDates($orders[$i]->getOrderDate()) > -3) { ?>
                                <td><strong style="color:orange">paid</strong></td>
                            <?php } else if (getDiffDates($orders[$i]->getOrderDate()) <= -2 && getDiffDates($orders[$i]->getOrderDate()) > -4) { ?>
                                <td><strong style="color:purple">in preparation</strong></td>
                                <?php } else if (getDiffDates($orders[$i]->getOrderDate()) <= -3 && getDiffDates($orders[$i]->getOrderDate()) > -5){ ?>
