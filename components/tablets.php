...
//we load all tablets from every categories
...
$sql = mysqli_query($succes, $sql = "SELECT * FROM `auctions` WHERE `secCategory` LIKE 'tablets'");
$row = mysqli_num_rows($sql);
while ($row > $compteur2) {
    $result = mysqli_fetch_assoc($sql);
...
$sql = mysqli_query($succes, $sql = "SELECT * FROM `bestoffers` WHERE `secCategory` LIKE 'tablets' AND `status` != 'complete'");
$row = mysqli_num_rows($sql);
while ($row > $compteur3) {
    $result = mysqli_fetch_assoc($sql);
...
$sql = mysqli_query($succes, $sql = "SELECT * FROM `flashsales` WHERE `secCategory` LIKE 'tablets'");
$row = mysqli_num_rows($sql);
while ($row > $compteur4) {
    $result = mysqli_fetch_assoc($sql);
