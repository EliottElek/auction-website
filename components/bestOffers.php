...
if (isset($_POST["submitresearch"]) && $_POST["submitresearch"]) {
  ...
  //now we load every bestoffer that is not 'complete'
...
while ($row > $compteur) {
    ...
    $creations[$compteur] = new bestoffer($result["category"], $result["brand"], $result["model"], $result["capacity"], $result["size"], $result["price"], $result["picture"], $result["description"], $result["id"], $result["sellerID"]);
    $idSeller = $result["sellerID"];
    $sql2 = mysqli_query($succes, $sql2 = "SELECT `firstname`, `lastname` FROM `users` WHERE `id` like '$idSeller'");
    $result2 = mysqli_fetch_assoc($sql2);
    $users[$compteur]=$result2["firstname"]." ". $result2["lastname"];
    $compteur = $compteur + 1;
}
if (!empty($creations)) {
    shuffle($creations);
...
<body>
    <section>
        <h3 style="align-self:baseline; margin:50px;"><a href="mainPage.php">Home</a>><a href="bestOffers.php">best offers</a></h3>
        <div class="showObjects">
            ...
            ?><div class="object">
                    <table>
                        ...
                        <tbody>
                            <tr>
                                <td><a href="selectedObject.php?Id=<?= $creations[$i]->getId(); ?>&category=bestoffers"><img class="image" src='<?php echo $creations[$i]->getPicture() ?>' alt=""></a></td>
                            </tr>
                            <tr>
                                </td>
                            </tr>
                            <tr>
                                <td>starting price :<?php echo $creations[$i]->getPrice() ?>£</td>
                            </tr>
                            <tr>
                                <td>sold by <?php echo $users[$i]?></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
