...
//now we get all sales
$compteur = 0;
$id = $_SESSION["user"]->getId();
$sql = mysqli_query($succes, $sql = "SELECT * FROM `bestoffers` WHERE 1");
...
while ($row > $compteur) {
  ...
  $bestoffers[$compteur] = new bestoffer($result["category"], $result["brand"], $result["model"], $result["capacity"], $result["size"], $result["price"], $result["picture"], $result["description"], $result["id"], $result["sellerID"]);
  $status[$compteur] = $result["status"];
  $compteur = $compteur + 1;
}
// Close statement
mysqli_close($succes);
...
<body>
    ...
    <br><br>
        <h3 id="welcome">ADMIN SESSION - Sales</h3>
        <br>
        <div class="myOrders">
        <h1>All sales</h1>
        <div class="myOrders">
        <table style="text-align: center; border: 1px solid black;margin-top:-40px;">
          <tbody>
            <tr>
            ...
            <?php for ($i = 0; $i < $compteur; $i++) { ?>
              <form action="" method="post">
                <tr>
                  <td><?php echo $bestoffers[$i]->getId() ?></td>
                  <td><?php echo $bestoffers[$i]->getPrice() ?>£</td>
                  <td style="color:red; text-transform:uppercase"><?php echo $bestoffers[$i]->getSellerId() ?></td>
                  <td colspan="3"><strong><?php echo $status[$i] ?></strong></td>
              </form>
            <?php } ?>
          </tbody>
        </table>
