...
// now we get every item from phones, tablets, laptops and earphones
...
$sql = mysqli_query($succes, $sql = $sql = "SELECT * FROM `tablets` WHERE 1");
$row = mysqli_num_rows($sql);
while ($row > $compteur3) {
    $result = mysqli_fetch_assoc($sql);
...
$sql = mysqli_query($succes, $sql = $sql = "SELECT * FROM `earphones` WHERE 1");
$row = mysqli_num_rows($sql);
while ($row > $compteur4) {
    $result = mysqli_fetch_assoc($sql);
...
<body>
    <section>
        <h3 style="align-self:baseline; margin:50px;"><a href="mainPage.php">Home</a>><a href="buyNow.php">Buy now</a></h3>
