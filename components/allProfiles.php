...
//now we get every account
$sql = mysqli_query($succes, $sql = "SELECT * FROM `users` WHERE 1");
...
while ($row > $compteur) {
    ...
    $creations[$compteur] = new user($result["id"], $result["firstname"], $result["lastname"], $result["email"], $result["isbuyer"]);
    $compteur = $compteur + 1;
}
// if (!empty($creations)) {
//     shuffle($creations);
// }
if (isset($_POST['deleteProfile']) && $_POST['deleteProfile']) {
    $index = $_POST['index'] - 1;
    ...
    if ($link->query($sql) === TRUE) {
        echo "user " . $creations[$index]->getFirstname() . " " . $creations[$index]->getFirstname() . " deleted successfully";
        header("refresh: 0");
    } else {
...
<head>
    ...
    <style>
        <?php include "../assets/allProfiles.css" ?><?php include "../assets/myAdresses.css" ?><?php include "../assets/mainPage.css" ?>
    </style>
...
<body>
    ...
    <input type="text" id="indexofcolor" value='<?php echo $indexOfColor ?>'>
        ...
        <br><br>
        <h3 id="welcome">ADMIN SESSION - Adresses</h3>
        <br><br>
            ...
            for ($i = 0; $i < $compteur; $i++) { ?>
                <div class="object">
                    <table>
                        <thead>
                            <tr>
                                <th><?php echo $creations[$i]->getFirstname() ?> <?php echo $creations[$i]->getLastname() ?></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td><img style="cursor:not-allowed" class="image" src='../images/userPic.png' alt=""></td>
                            </tr>
                            <tr>
                                <td><?php echo $creations[$i]->getEmail() ?></td>
                            </tr>
                            <tr>
                                <?php if ($creations[$i]->isBuyer()) { ?>
                                    <td>buyer : yes</td>
                                <?php } else { ?>
                                    <td>buyer : no</td>
                                <?php } ?>
                            ...
                            <tr>
                                <?php if ($creations[$i]->getEmail() == "eliott.morcillo@gmail.com") { ?>
                                    <td style="font-weight:900">ADMIN</td>
                                <?php } else { ?>
                                    <td><button class="deleteAccount">delete account</button></td>
                                <?php } ?>
                            </tr>
                        </tbody>
                    </table>
        ...
        <div id="myModal" class="modal">
            ...
            <div class="modal-content">
                ...
                <form action="" method="post">
                    <p>Are you sure you want to delete this account ? <input style="display:none" id="nbcard" name="index" value='1' /></p>
                    <input type="submit" onclick="delayReload()" class="btn-primary" name="deleteProfile" id="add" value="Yes, I want to">
                </form>
            </div>
        </div>
    </section>
...
<script>
    <?php include "../javascript/allProfiles.js" ?>
</script>
<script>
