...
//now we load every phones from buynow, auctions, flashsales and best offers;
...
$sql = mysqli_query($succes, $sql = "SELECT * FROM `auctions` WHERE `secCategory` LIKE 'phones'");
$row = mysqli_num_rows($sql);
while ($row > $compteur2) {
  $result = mysqli_fetch_assoc($sql);
...
$sql = mysqli_query($succes, $sql = "SELECT * FROM `bestoffers` WHERE `secCategory` LIKE 'phones' AND `status` != 'complete'");
$row = mysqli_num_rows($sql);
while ($row > $compteur3) {
  $result = mysqli_fetch_assoc($sql);
...
$sql = mysqli_query($succes, $sql = "SELECT * FROM `flashsales` WHERE `secCategory` LIKE 'phones'");
$row = mysqli_num_rows($sql);
while ($row > $compteur4) {
  $result = mysqli_fetch_assoc($sql);
      ...
      for ($i = 0; $i < $compteur; $i++) {
        ...
        <?php } else { ?>
          <div class="object">
            ...
            <?php } else {  ?>
              <img style="transform:translateX(150%)" class="iconpng" src="../images/vbuy_now.png" alt="">
            <?php } ?>
            <table>
              <thead>
                <tr>
